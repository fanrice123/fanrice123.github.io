...
>
> <div style={{textAlign: 'right'}}> -- <i>Steve Jobs</i></div>
...
You might argue that ***"Well I don't bother with that trade off since I have a machine that support 100 of threads to run in parallel."***, 
...
4. When case *2* and *3* happens at the same time...
...
```cpp:title=Non-atomic-assignment
...
And then, here comes another thread calling the `bar()` function. Who do you think the actual value it reads from `global_var`?
